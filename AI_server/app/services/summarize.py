...
async def summarize_first_half(input_data: STTRequest) -> str:
    ...
    이 발언의 핵심 주장을 한국어로 반드시 최대 {max_text_len}자 이내로 요약해 주세요.
    {max_text_len}자를 채우기 위해 없는 내용을 추가하지 말고 최대한 주어진 텍스트 내에서 가공하여 내용의 핵심을 요약해 주세요.
...
async def summarize_result_text(input_data: LastInput):
    ...
    for key, val in entire.items():
        ...
        이 토론을 해당 인물이 주장하고자 하는 바를 반드시 최대 300자 이내로 요약해주세요.
