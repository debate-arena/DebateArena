...
<template>
  <div class="icon-frame relative aspect-square overflow-visible isolate" ref="frameRef">
    ...
    <!-- 배지: 아이콘 위쪽에 표시 (bottom:100%) -->
    <transition name="badge-pop" appear>
      <div
        v-if="selected"
        class="absolute left-1/2 bottom-full -translate-x-1/2 -translate-y-4 h-10 md:h-11 px-6 rounded-2xl shadow-xl choice-plate pointer-events-none z-40 inline-flex items-center justify-center whitespace-nowrap text-center leading-none"
      >
        {{ badgeText }}
      </div>
    </transition>
